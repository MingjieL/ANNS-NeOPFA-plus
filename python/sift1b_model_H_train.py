...
def neopfa(train_data, train_query, test_data, test_query, kmeans_partitions, center_vectors,
    ...
    data = torch.from_numpy(train_data).float().to('cuda')
    train_query = torch.from_numpy(train_query).float().to('cuda')
    ...
    tt = torch.from_numpy(np.linspace(1, 10, topK)).float().to('cuda')
    best_recall = 0
    ...
    for epoch_i in range(epochs):
        ...
        for step in range(0, n_queries, batch_size):
            ...
            if iter1 % 2000 == 0:
                ...
                with torch.no_grad():
                    ...
                    for nk in range(n_kmeans):
                        data_embeddings = opHashNet(test_data[kmeans_partitions[nk]].to('cuda'))
                    ...
                    query_embeddings = opHashNet(test_query.to('cuda'))
                    data_embeddings_all = opHashNet(test_data.to('cuda'))
            ...
            gt_rank = torch.from_numpy(gt_rank).float().to('cuda')
            ...
            for i in range(batch_size):
                ...
                gt_relevance = relevance[gt_rank_i].to('cuda')
                ...
                for j in range(0, k2, n_step):
                    ...
                    loss1 = (torch.log((r - gt_rank_sub).pow(2) + 1) * gt_relevance[j:j + n_step]).sum() / k2
...
if __name__ == "__main__":
    data_name = "sift1b"
    output_folder = "/root/hdd-data/sift1b/Index_Learning_code"

    sift1b_base_path = '/root/hdd-data/sift1b/learn/base_data_1M.txt'
    sift1b_query_path = '/root/hdd-data/sift1b/learn/query_data_100.txt'
    sift1b_groundtruth_path = '/root/hdd-data/sift1b/learn/groundtruth_id_100.txt'

    sift1b_train_data = '/root/hdd-data/sift1b/learn/train_data_1.5M.txt'
    sift1b_train_query = '/root/hdd-data/sift1b/learn/train_query_700K.txt'

    # parameters:
    n_hidden = 512
    n_output = 25
    lr = 0.00005  # different dataset would need different learning rate
    epochs = 10
    batch_size = 50
    P = 1500000
    nq = 700000
    ...
    train_data = np.loadtxt(sift1b_train_data)
    # random_indices = torch.randperm(train_data.shape[0])
    # train_data = train_data[random_indices[:100000]]
    train_query = np.loadtxt(sift1b_train_query)
    test_data = np.loadtxt(sift1b_base_path)
    test_query = np.loadtxt(sift1b_query_path)
    test_gt = np.loadtxt(sift1b_groundtruth_path)
    ...
    kmeans_file = "/root/hdd-data/sift1b/learn/base_kmeans.txt"
