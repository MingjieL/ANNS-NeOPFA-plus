...
int main() {
    ...
    // int *flag = new int[n_num];
    // memset(flag, 0, sizeof(flag));
    char *new_pro_file = "/root/hdd-data/sift1b/pro_kmeans_data/sift1b_new_base_d25_v10_vv38_5.bin";
    char *K1_num_file = "/root/hdd-data/sift1b/pro_kmeans_data/kmeans_num_K1.txt";
    char *K2_num_file = "/root/hdd-data/sift1b/pro_kmeans_data/kmeans_num_K2.txt";
    char *ori_map_new_file = "/root/hdd-data/sift1b/pro_kmeans_data/ori_map_new.ds";
    char *new_map_ori_file = "/root/hdd-data/sift1b/pro_kmeans_data/new_map_ori.ds";
    ...
    for (int k = 0; k < K1; k++) {
        ...
        sprintf(base_file, "/root/hdd-data/sift1b/pro_kmeans_data/data_%d", k);
        sprintf(map_file, "/root/hdd-data/sift1b/pro_kmeans_data/map_%d.txt", k);
        sprintf(kmeans_file, "/root/hdd-data/sift1b/pro_kmeans_data/kmeans_%d.txt", k);
        ...
        // for (int i = 0; i < 10; i++) {
        //     unsigned long int offset = (unsigned long int)i * m_dimensions;
        //     for (int j = 0; j < m_dimensions; j++) {
        //         cout << new_data[offset + j] << " ";
        //     }
        //     cout << endl;
        // }
    }
    ...
    for (int i = 0; i < n_num; i++) {
        unsigned long int offset = (unsigned long int)i * m_dimensions;
        fwrite(new_data + offset, sizeof(float), m_dimensions, fp);
    }
    fclose(fp);
